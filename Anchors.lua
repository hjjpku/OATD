...
function Anchors:findPositive(roi_list, clip_rect, pos_threshold, neg_threshold, include_best)
  ...
  for i,roi in ipairs(roi_list) do
    ...
    for j,r in ipairs(ranges) do
      --[[
      if r.ys:size()[1]*r.xs:size()[1] > 0 then
        local anchor_rect = torch.Tensor(r.ys:size()[1]*r.xs:size()[1],4):fill(0):cuda()
        local ys = torch.repeatTensor(r.ys,r.xs:size()[1],1):cuda()
        anchor_rect[{{},{1,2}}] = ys
        for y=1,r.xs:size()[1] do
          anchor_rect[{{1+(y-1)*r.ys:size()[1], y*r.ys:size()[1]}, {3,4}}] = torch.repeatTensor(r.xs[{{y},{}}],1,r.ys:size()[1]):cuda()
        end
        local roi_rect = torch.Tensor{{roi.rect.minY, roi.rect.maxY, roi.rect.minX, roi.rect.maxX}}:cuda()
        roi_rect = torch.repeatTensor(roi_rect, r.ys:size()[1]*r.xs:size()[1], 1)

        local in_minx = torch.cmax(anchor_rect[{ {}, {3} }], roi_rect[ {{},{3}}])
        local in_maxx = torch.cmin(anchor_rect[{ {}, {4} }], roi_rect[ {{},{4}}])
        local in_miny = torch.cmax(anchor_rect[{ {}, {1} }], roi_rect[ {{},{1}}])
        local in_maxy = torch.cmin(anchor_rect[{ {}, {2} }], roi_rect[ {{},{2}}])
        local in_w = in_maxx - in_minx
        local in_h = in_maxy - in_miny

        local u_minx = torch.cmin(anchor_rect[{ {}, {3} }], roi_rect[ {{},{3}}])
        local u_maxx = torch.cmax(anchor_rect[{ {}, {4} }], roi_rect[ {{},{4}}])
        local u_miny = torch.cmin(anchor_rect[{ {}, {1} }], roi_rect[ {{},{1}}])
        local u_maxy = torch.cmax(anchor_rect[{ {}, {2} }], roi_rect[ {{},{2}}])
        local u_w = u_maxx - u_minx
        local u_h = u_maxy - u_miny

        local iou = torch.cdiv(torch.cmul(in_w,in_h),torch.cmul(u_w,u_h))
        local pos_anc_indx = torch.ge(iou,pos_threshold)
        pos_anc_indx = torch.nonzero(pos_anc_indx)

        if #pos_anc_indx:size() > 0 then -- not empty
          print('pos_num ' .. pos_anc_indx:size(1))
          for j=1,pos_anc_indx:size(1) do
            local idx = pos_anc_indx[j][1]
            local pos_anc = Rect.new(anchor_rect[idx][3], anchor_rect[idx][1], anchor_rect[idx][4], anchor_rect[idx][2])
            
            pos_anc.layer = r.layer
            pos_anc.aspect = r.aspect 
            pos_anc.index = { { r.aspect * 6 - 5, r.aspect * 6 }, r.ly + idx - math.floor(idx/r.ys:size()[1]) * r.ys:size()[1] - 1, r.lx + math.floor(idx/r.ys:size()[1])} -- index for rpn 1x1 conv result
            table.insert(matches, { pos_anc, roi })
          end
        end
      ...
      for y=1,r.ys:size()[1] do
        ...
        for x=1,r.xs:size()[1] do
          ...
          anchor_rect.index = { { r.aspect * 6 - 5, r.aspect * 6 }, r.ly + y - 1, r.lx + x - 1 } -- index for rpn 1x1 conv result
